...
«Сила Слова» — интерактивная образовательная драма для детей 10–16 лет. 40 эпизодов, 13 уроков критического мышления в форме сюжетного триллера. Сама игра живёт в **мобильном приложении** + **ботах в мессенджерах** (WhatsApp / Telegram / Viber) + email.
...
- **Подзаголовок:** «Интерактивная драма для детей 10–16 лет. Учит думать, а не верить.»
...
**ВЫБРАНА И ПОДКЛЮЧЕНА** (обновлено 2026-06-16; ранее в этом документе значилось «TBD»).

- **CRM:** собственная (кастомная) CRM заказчика на `crm.soroban.ua` (общая инфраструктура с проектом «Соробан»).
- **Эндпоинт:** inbound-вебхук `POST https://crm.soroban.ua/api/v1/crm/inbound/sila-slova-form`, адрес хранится в `CRM_WEBHOOK_URL` (env, не коммитится).
- **Авторизация:** секретный токен зашит в сам URL (capability-URL), отдельный заголовок `Authorization` НЕ требуется. Переменная `CRM_WEBHOOK_SECRET` — опциональный задел на случай перехода CRM на header-аутентификацию (см. §11.4).
- **Реализация на сайте:** `src/app/api/enroll/route.ts` принимает заявку с формы, валидирует серверной схемой `src/lib/schemas/enroll.ts` (allowlist полей + лимиты длины), затем пересылает в CRM с таймаутом 8 с.

> Историческая справка (кандидаты, рассматривавшиеся до выбора): HubSpot, Pipedrive, ActiveCampaign, AmoCRM, Customer.io, Sendpulse / Manychat.

**Требование к фронту (по-прежнему в силе):** webhook-клиент остаётся конфигурируемым через env — смена CRM не должна требовать правок кода, только переменных окружения.
...
> **Факт на 2026-06-16:** ниже — изначальный обобщённый контракт. Реально подключённая
> CRM (`crm.soroban.ua`, см. §11.2) авторизует запрос **токеном в URL**, а не заголовком
> `Authorization: Bearer` — заголовок сейчас не отправляется. Фактический формат тела
> заявки тоже отличается (`{ payload: { title, phones[], source, city, comment?,
> source_information?, utm_* } }`) — см. `src/lib/schemas/enroll.ts`.
...
- 5–7 фич приложения (бейджи: «10–16 yo», «no ads», «offline play», «parent dashboard»)
- QR-код на десктопе
...
\* **Пересмотрено 2026-05.** Исходный таргет ≤ 80 КБ
...
| #   | Что                                                                                                                     | Когда        |
| --- | ----------------------------------------------------------------------------------------------------------------------- | ------------ |
| 1   | ✅ **СДЕЛАНО** — CRM выбрана (своя `crm.soroban.ua`), webhook URL получен и подключён (см. §11.2). Авторизация — токеном в URL  | ~~До спринта 3~~ |
